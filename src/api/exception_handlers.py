...
def exception_registry(app: FastAPI) -> None:
    ...
    def handle_chat_not_found_exception(request: Request, exc: ChatNotFoundException):
        ...
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND, content={"message": f"{exc.message}"}
        )

    @app.exception_handler(MessageNotFoundException)
    def handle_message_not_found_exception(
        request: Request, exc: MessageNotFoundException
    ):
        logger.error("%s: %s", exc.__class__.__name__, exc.message)
    ...
    def handle_chat_permissions_not_found_exception(
        request: Request, exc: ChatPermissionsNotFoundException
    ):
        logger.error("%s: %s", exc.__class__.__name__, exc.message)
    ...
    def handle_sqlalchemy_error_exception(request: Request, exc: SQLAlchemyError):
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"message": "SQLAlchemy error"},
        )
